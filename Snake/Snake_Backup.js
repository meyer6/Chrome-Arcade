	    	...
	    	if (Board[y][x]!==null){
	    		...
	    		if (y==body[0][0] && x==body[0][1]){
	    			if (body_direction[0][0]==0 && body_direction[0][1]==1){
		    			var headr = new Image()
						headr.src = "headr.png"
						console.log(headr,0,0,128,128,x*(30)+18+6*frame*x_deviation+x_deviation2,y*(30)+8+31+6*frame*y_deviation+y_deviation2,128,128)
	    			...
	    			if (body_direction[0][0]==-1 && body_direction[0][1]==0){
		    			var headu = new Image()
						headu.src = "headu.png"
						ctx.drawImage(headu,0,0,128,128,x*(30)+18-9+6*frame*x_deviation+x_deviation2,y*(30)+8+11+6*frame*y_deviation+y_deviation2,128,128);
	    			}
...
if (typeof(Storage) !== "undefined") {
	...
	if (High_Score==null){
		...
		var High_Score=window.localStorage.getItem('High_score');
		var Death=window.localStorage.getItem('Death');
		if (Death=="true"){
			Death=true
		}else{
			Death=false
...
function gameLoop(){
	if (once==true || keys[87] || keys[83] || keys[65] || keys[68] || keys[32]){
		...
		sleep(20).then(() => { 
			...
			window.localStorage.setItem('Death',Death.toString());
