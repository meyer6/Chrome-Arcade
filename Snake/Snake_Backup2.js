class Snake{
	...
	move_forward(){
		if (move_finished==true){
            ...
            if (end-start>=100){
				if (keys[87] && Snake_class.direction[0]!==1){
					Snake_class.direction=[-1,0];
					keys[87]=false;
				}else if (keys[83] && Snake_class.direction[0]!==-1){
					Snake_class.direction=[1,0];
					keys[83]=false;
				}else if (keys[68] && Snake_class.direction[1]!==-1){
					Snake_class.direction=[0,1];
					keys[68]=false;
				}else if (keys[65] && Snake_class.direction[1]!==1){
					Snake_class.direction=[0,-1];
					keys[65]=false;				
				}
            	frame=1;
	    	...
	    	if (Board[y][x]!==null){
	    		...
	    		if (y==body[0][0] && x==body[0][1]){
	    			...
	    			for (i=1;i<body.length-1;i++){
						if (body.length<37){
							var colour2=(body_colours[i-1]);
		            	}else{
		            		var colour2=(body_colours[Math.floor((36/body.length)*(i-1))]);
		            	}		
	    				if (y==body[i][0] && x==body[i][1]){
	    					if ((body_direction[i-1][0]!==body_direction[i][0] && body_direction[i-1][1]!==body_direction[i][1]) ||(body_direction[i][0]!==body_direction[i+1][0] && body_direction[i][1]!==body_direction[i+1][1])){
								var y_deviation=Board_directiony[body[body.length-2][0]][body[body.length-2][1]]
								var x_deviation=Board_directionx[body[body.length-2][0]][body[body.length-2][1]]
								var x_deviation2=0
								var y_deviation2=0
	    						...
	    						pass=true;
	    						turned=false
			    				if (body_direction[a][0]==0 && body_direction[a][1]==1 && body_direction[a+1][0]==1 && body_direction[a+1][1]==0){	
			    					var cx=x*(30)+48
			    					var cy=y*(30)+18+31
			    					var start_angle=1.5708
			    					var end_angle=1.5708*2
			    					var sx=x*(30)+18+30
			    					var sy=y*(30)+18+31
			    					var dx=x*(30)+18
			    					var dy=y*(30)+18+1
			    					var fx=0
			    					var fy=17+(6*frame*y_deviation+y_deviation2+14)
			    					var x_size=30
			    					var y_size=-(6*frame*y_deviation+y_deviation2)
								}else if (body_direction[a][0]==0 && body_direction[a][1]==1 && body_direction[a+1][0]==-1 && body_direction[a+1][1]==0){	
			    					var cx=x*(30)+48
			    					var cy=y*(30)+48+31
			    					var start_angle=1.5708*2
			    					var end_angle=1.5708*3
			    					var sx=x*(30)+18+30
			    					var sy=y*(30)+18+31
			    					var dx=x*(30)+18
			    					var dy=y*(30)+78+1
			    					var fx=0
			    					var fy=17+(6*frame*y_deviation+y_deviation2+14)-32
			    					var x_size=30
			    					var y_size=-(6*frame*y_deviation+y_deviation2)
								}else if (body_direction[a][0]==0 && body_direction[a][1]==-1 && body_direction[a+1][0]==1 && body_direction[a+1][1]==0){	
			    					var cx=x*(30)+48-30
			    					var cy=y*(30)+48+31-30
			    					var start_angle=1.5708*4
			    					var end_angle=1.5708*5
			    					var dx=x*(30)+18
			    					var dy=y*(30)+18+31-30
			    					var sx=x*(30)+18-30
			    					var sy=y*(30)+78+1-30
			    					var fx=0
			    					var fy=17+(6*frame*y_deviation+y_deviation2+14)
			    					...
			    					var cx=x*(30)+18
			    					var cy=y*(30)+48+31
			    					var start_angle=1.5708*3
			    					var end_angle=1.5708*4
			    					var sx=x*(30)+18-30
			    					var sy=y*(30)+18+31
			    					var dx=x*(30)+18
			    					var dy=y*(30)+78+1
			    					...
			    					var x_size=30
			    					var y_size=-(6*frame*y_deviation+y_deviation2)
								}else if (body_direction[a][0]==-1 && body_direction[a][1]==0 && body_direction[a+1][0]==0 && body_direction[a+1][1]==1){	
			    					var cx=x*(30)+18
			    					var cy=y*(30)+18+31
			    					var start_angle=1.5708*4
			    					var end_angle=1.5708*5
			    					...
			    					var sx=x*(30)+18
			    					var sy=y*(30)+18+1
			    					var fx=17+(6*frame*x_deviation+x_deviation2+14)
			    					var fy=0
			    					var x_size=-(6*frame*x_deviation+x_deviation2)
			    					var y_size=30
								}else if (body_direction[a][0]==1 && body_direction[a][1]==0 && body_direction[a+1][0]==0 && body_direction[a+1][1]==1){	
			    					var cx=x*(30)+18
			    					var cy=y*(30)+18+61
			    					...
			    					var sx=x*(30)+18
			    					var sy=y*(30)+78+1
			    					var fx=17+(6*frame*x_deviation+x_deviation2+14)-30
			    					var fy=0
			    					var x_size=-(6*frame*x_deviation+x_deviation2)
			    					var y_size=30
								}else if (body_direction[a][0]==-1 && body_direction[a][1]==0 && body_direction[a+1][0]==0 && body_direction[a+1][1]==-1){	
				            	...
				            	if (Board[y][x]==colour2)	{
				            	}
								ctx.fillStyle=Board[y][x];
			                	ctx.moveTo(cx, cy);
								ctx.arc(cx,cy,30,start_angle,end_angle);
								ctx.lineTo(cx,cy);
								ctx.fill() 
								if (a==body.length-3 || a==body.length-2 || a==body.length-1 || a==body.length){
									ctx.fillRect(dx+fx,dy+fy, x_size,y_size);
									turned=true;
								}else{
									if(body_direction[i+1]==body_direction[i]){
										ctx.fillRect(dx,dy, 30, 30);
									}
								}
								if(sx!==body[0][1]*30+18 || sy!==body[0][0]*30+31+18){
									if(body_direction[i-1]==body_direction[i-2]){
	    			...
	    			if (pass==false || turned==true){
						ctx.fillStyle=Board[y][x];
						ctx.fillRect(x*(30)+18+6*frame*x_deviation+x_deviation2, y*(30)+18+31+6*frame*y_deviation+y_deviation2, 30, 30);
					}
				}
			}
		}
...
var once=false;
function gameLoop(){
	if (once==true || keys[87] || keys[83] || keys[65] || keys[68] || keys[32]){
		once=true;
		sleep(20).then(() => { 
			if (points==284){
				points=285;
