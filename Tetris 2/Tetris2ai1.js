...
function All_Moves(){
    ...
    for(r=0;r<4;r++){
        for(m=-5;m<6;m++){
            Pieces_joined=[[1,[4,1,4,2,4,3,5,3]], [2,[5,1,5,2,5,3,4,3]], [3,[4,0,4,1,4,2,4,3]], [4,[4,2,4,3,5,2,5,3]], [5,[4,1,4,2,5,2,5,3]], [6,[5,1,5,2,4,2,4,3]], [7,[4,1,4,2,4,3,5,2]]]
            test_piece=new Piece(Pieces_joined[current_piece.colour-1][0], Pieces_joined[current_piece.colour-1][1]); 
            Pieces_joined=[[1,[4,1,4,2,4,3,5,3]], [2,[5,1,5,2,5,3,4,3]], [3,[4,0,4,1,4,2,4,3]], [4,[4,2,4,3,5,2,5,3]], [5,[4,1,4,2,5,2,5,3]], [6,[5,1,5,2,4,2,4,3]], [7,[4,1,4,2,4,3,5,2]]]
            ...
            var prediction_reset=prediction
            test_piece.Predict()
            prediction.slice(prediction.length-1,1)
            var prediction_save=prediction
            prediction=prediction_reset

            Remove(test_piece.blocks,test_piece.colour)
            Add(prediction_save,test_piece.colour)
            var score=Analyze_Position(board,test_piece.colour)
            moves.push([r,m,score,0])
            Remove(prediction_save)
    ...
    if(holding_piece!=0){
        for(r=0;r<4;r++){
            for(m=-5;m<6;m++){
                Pieces_joined=[[1,[4,1,4,2,4,3,5,3]], [2,[5,1,5,2,5,3,4,3]], [3,[4,0,4,1,4,2,4,3]], [4,[4,2,4,3,5,2,5,3]], [5,[4,1,4,2,5,2,5,3]], [6,[5,1,5,2,4,2,4,3]], [7,[4,1,4,2,4,3,5,2]]]
                test_piece=new Piece(Pieces_joined[holding_piece.colour-1][0], Pieces_joined[holding_piece.colour-1][1]); 
                Pieces_joined=[[1,[4,1,4,2,4,3,5,3]], [2,[5,1,5,2,5,3,4,3]], [3,[4,0,4,1,4,2,4,3]], [4,[4,2,4,3,5,2,5,3]], [5,[4,1,4,2,5,2,5,3]], [6,[5,1,5,2,4,2,4,3]], [7,[4,1,4,2,4,3,5,2]]]
        ...
        for(r=0;r<4;r++){
            for(m=-5;m<6;m++){
                Pieces_joined=[[1,[4,1,4,2,4,3,5,3]], [2,[5,1,5,2,5,3,4,3]], [3,[4,0,4,1,4,2,4,3]], [4,[4,2,4,3,5,2,5,3]], [5,[4,1,4,2,5,2,5,3]], [6,[5,1,5,2,4,2,4,3]], [7,[4,1,4,2,4,3,5,2]]]
                test_piece=new Piece(Pieces_joined[next_piece.colour-1][0], Pieces_joined[next_piece.colour-1][1]); 
                Pieces_joined=[[1,[4,1,4,2,4,3,5,3]], [2,[5,1,5,2,5,3,4,3]], [3,[4,0,4,1,4,2,4,3]], [4,[4,2,4,3,5,2,5,3]], [5,[4,1,4,2,5,2,5,3]], [6,[5,1,5,2,4,2,4,3]], [7,[4,1,4,2,4,3,5,2]]]
                ...
                Add(prediction_save,test_piece.colour)
                var score=Analyze_Position(board,test_piece.colour)
                moves.push([r,m,score,1,test_piece])
                Remove(prediction_save)
            }
        }        
    }
    Add(current_piece.blocks,current_piece.colour)
    ...
    for(i=0;i<moves[0][0];i++){
        current_piece.Rotate()
    }
    if(moves[0][1]<0){
        for(i=0;i<-moves[0][1];i++){
            current_piece.Left(); 
        }                
    }else if(moves[0][1]>0){
        for(i=0;i<moves[0][1];i++){
...
function Analyze_Position(board2,colour){
    var score=100

    for(x=0;x<10;x++){
        for(y=4;y<21;y++){
            if(board2[x+y*10]!="0" && board2[x+(y+1)*10]=="0"){
                score=score-(21-y)*20
            }
        }
        for(y=4;y<22;y++){
    ...
    for(x=0;x<9;x++){
        for(y=4;y<20;y++){
            if(board2[x+y*10]=="0" && board2[x+(y+1)*10]=="0" && board2[x+(y+2)*10]=="0"){
                if(board2[(x-1)+y*10]!="0" || x-1<0){
                    if(board2[(x+1)+y*10]!="0" || x+1>8){
                        score=score-100000
                    }
                }
            }
    ...
    if(colour==3){
        for(x=0;x<10;x++){
            for(y=4;y<21;y++){
                if(board2[x+y*10]!="0"){
                    board2=board2.slice(0,x+y*10)+"1"+board2.slice(x+y*10+1,220)
                }
            }
        }
        if(board2.indexOf("1111111111111111111111111111111111111111")!=-1){
        ...
        for(y=4;y<22;y++){
            if(board2[9+y*10]!="0"){
                score=score=score-(21-y)*10
            }
        }
    }
    if(board2.slice(0,40)!="0000000000000000000000000000000000000000"){
...
async function gameLoop(){
    if(death==false){//Date.now()-start>1
        if(frame%1==0){
            move=0
            move_y=0
            if(keys[65]==true){
        ...
        //await sleep(Math.max(1-(Date.now()-start),0));
    //}
