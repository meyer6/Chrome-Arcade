function Game_State_Display(){
    ...
    for(i=mines.length-1;i>=0;i--){
        var colour=mine_colours[Math.floor((Date.now()-mines[i][2])*39/6000)]
        var pass=true
        if(Date.now()-mines[i][2]>=200){
            for(x=-6;x<=6;x=x+2){
                for(y=-6;y<=6;y=y+2){
                    var imgData = ctx.getImageData(mines[i][0]+x, mines[i][1]+y, 1, 1);
                    imgData=rgbToHex(imgData.data[0],imgData.data[1],imgData.data[2])
                    if(imgData!=undefined && imgData!="#000000" && imgData!="#fffb00" && imgData!="#ffffff"){
                        pass=false
                        console.log(imgData)
                    }
                }
        ...
        if(Date.now()-mines[i][2]>=6000 || pass==false){
            drawCircle(mines[i][0],mines[i][1], 22, "#FFFFF2", "#FFFFF2", 22)
            mines.splice(i,1)
        }else{
            drawCircle(mines[i][0],mines[i][1], 6*Math.min((Date.now()-mines[i][2]),300)/300, "#FFFFF2", "#FFFFF2", 6*Math.min((Date.now()-mines[i][2]),300)/300)
            drawCircle(mines[i][0],mines[i][1], 3.5*Math.min((Date.now()-mines[i][2]),300)/300, colour, colour, 3.5*Math.min((Date.now()-mines[i][2]),300)/300)
        }
    }
    for(i=0;i<2;i++){
        ctx.lineWidth = 20;
...
function mine(){
    ...
    if(keys[87] && frequency[0][0]==0 || keys[87] && Date.now()-mines[frequency[0][1]][2]>1000 && frequency[0][0]<2){
        mines.push([jets[0][0]-jets[0][2]*4,jets[0][1]-jets[0][3]*4,Date.now(),0])
    }
    if(keys[40] && frequency[1][0]==0 || keys[40] && Date.now()-mines[frequency[1][1]][2]>1000 && frequency[1][0]<2){
        mines.push([jets[1][0]-jets[1][2]*4,jets[1][1]-jets[1][3]*4,Date.now(),1])
    }   
}
